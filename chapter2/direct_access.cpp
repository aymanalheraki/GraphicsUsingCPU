...
void direct_read_pixels(SDL_Surface* surface){
  uint32_t* pixelsArray = (uint32_t*)surface->pixels;
  int pitch = surface->pitch / 4;
  for(int y = 0; y < surface->h; ++y){
    for( int x = 0; x < surface->w; ++x){
      uint32_t pixel = pixelsArray[y*pitch + x];
...
void direct_fill_white(SDL_Surface* surface){
  uint32_t* pixelsArray = (uint32_t*)surface->pixels;
  int pitch = surface->pitch / 4;
  for(int y = 0; y < surface->h; ++y){
    for( int x = 0; x < surface->w; ++x){
      // Will use rgba format instead of int
      pixelsArray[y * pitch + x] = 4294967295;
    }
  }
}
...
int main(int argc, char** args) {
  ...
  direct_fill_white(surface);

  SDL_UpdateWindowSurface( window );
  
  // Direct memory access pixel reading
  direct_read_pixels(surface);
